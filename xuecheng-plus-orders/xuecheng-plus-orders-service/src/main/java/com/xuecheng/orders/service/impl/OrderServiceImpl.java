...
public class OrderServiceImpl implements OrderService {
    ...
    public void notifyPayResult(MqMessage message) {
        ...
        Long id = message.getId();//消息ID
        //全局消息ID(不重覆)
        CorrelationData correlationData = new CorrelationData(id.toString());
        //使用coorelationData指定回調方法
        correlationData.getFuture().addCallback(result -> {
            if (result.isAck()) {
                //消息成功發送到交換機,並將消息從數據庫表刪除
                log.debug("發送消息成功:{}",jsonString);
                mqMessageService.completed(id);
            } else {
                //消息發送失敗
                log.debug("發送消息失敗:{}",jsonString);
            }
        }, ex -> {
            //網路異常
            log.debug("發送消息異常:{}",jsonString);
        });

        rabbitTemplate.convertAndSend(PayNotifyConfig.PAYNOTIFY_EXCHANGE_FANOUT, "",
                messageObj, correlationData);
    }
}
